...
function pprint($arr, $name = 1, $type = 1, $printable = 1)
{
    global $style, $height;
    $bt     = debug_backtrace();
    ...
    echo '<label style='.$style['label'].' onclick='.$style['hideContent'].'>
			...
			<span style='.$style['key'].'>$'.$name.'</span>
...
function pprint_array($arr, $p, $printable, $type)
{
    ...
    switch ($t) {
        ...
        case 'array':
            echo '<span style='.$style['array'].'></span>'.$arround['array_1'].(($type) ? ' <span style='.$style['type'].'>('.count($arr).')</span>' : '')."\r\n";
